// ==UserScript==
// @name Shack-lol
// @namespace https://www.lmnopc.com/greasemonkey/
...
/*
	...
	https://lmnopc.com/greasemonkey/shack2007/usemyamazonaffiliatecode.user.js
	...
	https://wishlistbuddy.com/user/thomw
...
(function() {
	...
	function lolThread(tag, id)
	{
		...
		var addr = 'https://' + myDomain + '/greasemonkey/shacklol/report.php?who=' + encodeURIComponent(username) + '&what=' + encodeURIComponent(id) + '&tag=' + encodeURIComponent(tag) + '&version=' + encodeURIComponent(version) + moderation;
			...
			onload: function(result) {
				...
				try
				{
					...
					else
					{
						...
						objLol.innerHTML = '<a href="https://' + myDomain + '/greasemonkey/shacklol/?user=' + encodeURIComponent(username) + '" style="color: #f00;">' + taggd + '</a>';
					}
				}
				catch (e)
				{
	...
	{
		var addr = 'https://' + myDomain + '/greasemonkey/shacklol/api.php?thread_id=' + encodeURIComponent(id) + '&tag=' + tag_name + '&special=get_taggers';
	...
	function retrieveLolCounts()
	{
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'https://lmnopc.com/greasemonkey/shacklol/api.php?special=getcounts',
	...
	function installApril1Button(threadIdList)
	{
		...
		for (var i = 0, ii = threadIdList.length; i < ii; i++)
		{
			...
			divLol.addEventListener('click', function() {
				...
				audioElement.setAttribute('src', 'https://gamewith.us/shackspace/sadtuba.ogg');
				audioElement.play();
				alert('APRIL FOOLS!');
			}, false);
	...
	*/
	...
	if (String(location.href).indexOf('.com/chatty') != -1)
	{
		...
		if (divCommentstools != null)
		{
			...
			divLol.setAttribute('href', 'https://' + myDomain + '/greasemonkey/shacklol/?user=' + encodeURIComponent(username));
